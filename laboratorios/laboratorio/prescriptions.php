...
<?php 	
	...
	if(isset($_POST['submit'])){
		...
		$recetas = prescription::recetas_por_producto($fecha_ini,$fecha_fin,$producto,$laboratorio->id);
	...
	if(isset($_POST['submit_general'])){
		...
		$recetas_generales = prescription::recetas_por_laboratorio($fecha_ini,$fecha_fin,$laboratorio->id);		
	}
?>
...
<head>
	...
	<title>iRecetas - Sección Laboratorios - iMedics.ws - Internet Medical Solutions</title>
		...
		<div id="header" class="clearfix">
			...
			<div id="nav" class="clearfix">				
				<ul>
					...
					<li><a class="selected" href="prescriptions.php">iReceta</a></li>
		...
		<div id="content" class="clearfix">
			...
			<div id="center-login" class="clearfix">				
				...
				<div id="optionlabs">
					...
					<hgroup>
					<h1>Listado de <span>i</span>Recetas</h1>
					</hgroup>
					<?php if(count($medicamentos) != 0){ ?>				
						<table border="1" width="100%">
							<tr>
								<td colspan="2" style="color:#000;font-size:12px;">*Seleccione las fechas para visualizar las recetas</td>
							</tr>
							<?php $o = 0; ?>
							...
							<tr>
								...
								<td width="50%" class="estilo<?php echo $j; ?>">
									...
									<form id="visits" action="prescriptions.php" method="post">
										<label for="Del">Del:
										<?php $o = $o+1; ?>
										<script>
				...
				<div id="options">
					...
					<hgroup>
					<h1>Listado de <span>i</span>Recetas <?php echo isset($medicament) ? htmlentities($medicament->nombre) : ""; ?></h1>
					</hgroup>
					
					<table width="100%" border="1">
					<?php if(isset($recetas)){ ?>
							<tr>
								<td style="color:#000;">Nombre del m&eacute;dico que receto</td>
								<td style="color:#000;">Especialidad</td>
								<td style="color:#000;">F. Receta</td>
							</tr>
							...
							<?php } ?>
							<tr>
								<td style="color:#000;font-size:12px;" colspan="3">Este medicamento a sido recetado <?php echo count($recetas); ?> veces<br />Del: <?php echo $fecha_ini; ?> Al: <?php echo $fecha_fin; ?></td>
							</tr>
						<?php } ?>
						
					<?php if(isset($recetas_generales)){ ?>
							<tr>
								<td style="color:#000;">Nombre del m&eacute;dico que receto</td>
								<td style="color:#000;">Especialidad</td>
								<td style="color:#000;">F. Receta</td>
							</tr>
							<?php foreach($recetas_generales as $recetag){ ?>
								<?php $medico = medic::buscar_por_id($recetag->medico_id); ?>
								<td><?php echo $medico->nombre; ?></td>
								<?php $especialidad = speciality::buscar_por_id($medico->especialidad_id); ?>
								<td style="color:#069;"><?php echo htmlentities($especialidad->nombre); ?></td>
								<td><?php echo $recetag->fecha; ?></td>
							</tr>
							<?php } ?>
							<tr>
								<td style="color:#000;font-size:12px;" colspan="3">Existen un total de <?php echo count($recetas_generales); ?> recetas<br />Del: <?php echo $fecha_ini; ?> Al: <?php echo $fecha_fin; ?></td>
							</tr>
						<?php } ?>
					</table>
