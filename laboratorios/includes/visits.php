...
class visit extends database_table
{
	...
	public function visitas_por_producto($fecha_ini,$fecha_fin,$producto,$lab){
		$sql = "SELECT * FROM ".static::$nombre_tabla." WHERE fecha between '{$fecha_ini}' and '{$fecha_fin}'";
		$sql .= " AND laboratorio_id={$lab} AND producto_id={$producto} ORDER BY fecha ASC";
		return $resultado = visit::buscar_por_sql($sql);
	}
	
	public function visitas_por_laboratorio($fecha_ini,$fecha_fin,$lab){
		$sql = "SELECT * FROM ".static::$nombre_tabla." WHERE fecha between '{$fecha_ini}' and '{$fecha_fin}'";
