...
class database_table
{
	...
	public static function cantidad_total($menu_id)
	{
		global $bd;
	...
	public static function total_secciones($seccion_id)
	{
		global $bd;
        $sql = "SELECT COUNT(*) FROM ".static::$nombre_tabla;
	...
	public static function total_subsecciones($subseccion_id)
	{
		global $bd;
        $sql = "SELECT COUNT(*) FROM ".static::$nombre_tabla;
		$sql .= " WHERE subseccion_id={$subseccion_id}";
		$sql .= " AND estado='activo'";
        $resultado = $bd->enviar_consulta($sql);
        $registro = $bd->fetch_array($resultado);
        return array_shift($registro);
