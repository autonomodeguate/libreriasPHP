...
<?php
	...
	$section = sections::buscar_por_id($_GET['section']);
	
	//$productos = product::productos_por_seccion($_GET['section']);
	
	/*Paginacion*/
	$pagina = (!empty($_GET["pagina"])) ? (int)$_GET["pagina"] : 1;
	$productos_grupo = 15;
	$total_productos = product::total_secciones($_GET['section']);
	
	$paginacion = new pagination($pagina,$productos_grupo,$total_productos);
	$sql = "SELECT * FROM productos ";
	$sql .= " WHERE seccion_id=".$_GET['section'];
...
<head>
	...
	<title><?php echo $section->nombre; ?> - Ofertas Chapinas.com - Una Manera Sencilla de Comprar</title>
		...
		<section id="content" class="clearfix">
			...
			<aside id="aside">
				...
				<?php foreach($secciones as $seccion){ ?>
					<li><a <?php if($seccion->id == $section->id){ echo ' class="section"';} ?> href="sections-iteam.php?section=<?php echo urlencode($seccion->id); ?>&iteam=<?php echo urlencode($seccion->menu_id); ?>&name=<?php echo urlencode($seccion->nombre); ?>"><?php echo $seccion->nombre; ?></a>
						<ul>
							<?php $subsecciones = subsections::obtener_subsecciones_por_seccion($seccion->id); ?>
							<?php foreach($subsecciones as $subseccion){ ?>
								<li><a href="subsections.php?subsection=<?php echo urlencode($subseccion->id); ?>&iteam=<?php echo urlencode($seccion->menu_id); ?>&name=<?php echo $subseccion->nombre; ?>"><?php echo $subseccion->nombre; ?></a></li>
							<?php } ?>
						</ul>
					</li>
				<?php } ?>
			...
			<section id="section">
				...
				<div id="pagination">
					<?php
						...
						{
							echo "&nbsp;<a href=\"sections-iteam.php?iteam=".urlencode($producto->menu_id)."&section=".urlencode($producto->seccion_id)."&pagina=";
						...
						for($i=1;$i<=$paginacion->total_paginas();$i++)
						{
							...
							{
								echo "&nbsp;<a href=\"sections-iteam.php?iteam=".urlencode($producto->menu_id)."&section=".urlencode($producto->seccion_id)."&pagina={$i}\">$i</a>&nbsp;";
							}
						}
